    ...
    try:
        ...
        if not complete:
            if not file.error_message:
                ...
                #
                if is_file is None :
                   msgg = f"📶 : {file.connections} <b>"
                else :
                   msgg = f"🌎 : {file.connections} | 🌱 : {file.num_seeders} <b>"
                msg = f"\n`{downloading_dir_name}`"
                msg += f"\n<b>🚀</b> : {file.download_speed_string()}"
                msg += f"\n<b>💽</b> : {file.progress_string()} <b>of</b> {file.total_length_string()} <b>"
                msg += f"\n</b>{msgg}"
                msg += f"\n</b>⏳ : {file.eta_string()} <b>"
                msg += f"\n</b>📝 : <code>{gid}</code>"
                ...
                #else :
                   #msg += f"\n<b>🔎 :</b>[ 🌎 : {file.connections} | 🌱 : {file.num_seeders} ], 📝: <code>{gid}</code>"
